...
describe('PrismaCategoryRepository (integración, sicfi-dev)', () => {
  ...
  beforeAll(async () => {
    ...
    repo = new PrismaCategoryRepository(scopedPrisma);
  });

  afterAll(() => household.cleanup());

  it('guarda y recupera una categoría contra Postgres real', () =>
    household.run(async () => {
  ...
  it('findByName respeta el aislamiento por householdId', () =>
    household.run(async () => {
  ...
  it('update() persiste los cambios y delete() borra la fila', () =>
    household.run(async () => {
      const category = new Category({
